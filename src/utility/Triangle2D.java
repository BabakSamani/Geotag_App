...
// 		double slopeBC = -1 / Slope(b, c);
 		double slopeAB = Slope(midAB, c);
 		double slopeBC = Slope(midBC, a);
 		...
 		Vector2D circumcenter = new Vector2D(
 			x,
 			(slopeAB * x) + bAB
 		);
...
//    	double y2=b.y;
//    	double y3=c.y;
//    	double xNumerator=(Math.pow(x1,2)+Math.pow(y1,2)-Math.pow(x2,2)-Math.pow(y2,2))*(y2-y1)-(Math.pow(x2,2)+Math.pow(y2,2)-Math.pow(x3,2)-Math.pow(y3,2))*(y1-y2);
//    double xDenominator=2*((x1-x2)*(y2-y3)-(x2-x3)*(y1-y2));
//    	double x=xNumerator/xDenominator;
//    	double y=(x2+x3)/2+((x3-x2)/(y2-y3))*x+(Math.pow(x2,2)-Math.pow(x3,2))/(2*(y2-y3));
